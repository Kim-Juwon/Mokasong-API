...
public class UserRegisterDto {
    ...
    @NotBlank(groups = Register.class, message = "이메일은 필수입니다.")
    @Email(groups = Register.class, message = "이메일 형식이어야 합니다.")
    private String email;

    @Schema(description = "비밀번호", required = true, nullable = false)
    // TODO: password pattern 정의하기
    @NotBlank(groups = Register.class, message = "비밀번호는 필수입니다.")
    private String password;

    @Schema(description = "휴대폰 번호", required = true, nullable = false, example = "01023911319")
    @NotBlank(groups = Register.class, message = "휴대전화번호는 필수입니다.")
    @Pattern(groups = Register.class, regexp = "^010\\d{7,8}$", message = "휴대전화번호 형식이어야 합니다. (010xxxxxxx 또는 010xxxxxxxx)")
    private String phone_number;

    @Schema(description = "이름", required = true, nullable = false, example = "김주원")
    // TODO: name pattern 한번 더 고민하기
    @NotBlank(groups = Register.class, message = "이름은 필수입니다.")
    @Pattern(groups = Register.class, regexp = "^[가-힣|a-z|A-Z]+$", message = "이름이 유효하지 않습니다.")
    private String name;
